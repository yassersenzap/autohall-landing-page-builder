/** Mirror of backend/src/landing-render/block-design-system.ts */
import {
...
export function getDefaultDesignForBlock(blockType: string): Partial<NormalizedBlockDesign> {
  return {
    variant: 'standard',
    tone: 'light',
    density: 'comfortable',
...
export function normalizeSectionDesign(
  ...
  return {
    variant: pickVariant(blockType, raw.variant, blockDefaults.variant ?? 'standard'),
    tone: pick(raw.tone, TONES, blockDefaults.tone ?? 'light'),
    density: premium.density,
    mediaPosition: premium.mediaPosition,
    imageShape: premium.imageShape,
    ctaStyle: premium.ctaStyle,
    alignment: pick(raw.alignment, ALIGNMENTS, blockDefaults.alignment ?? 'left'),
  };
}

export function buildBlockDesignClasses(baseClass: string, design: NormalizedBlockDesign): string {
  const premiumLike = {
    ...design,
  ...
  if (design.tone === 'neutral') classes.push(`${baseClass}--tone-neutral`);
  return classes.join(' ');
}

export function buildBlockCtaClass(design: NormalizedBlockDesign, base = 'lp-btn lp-btn--lg'): string {
...
export function patchBlockDesign(
  propsJson: Record<string, unknown>,
  blockType: string,
  patch: Partial<NormalizedBlockDesign>,
): Record<string, unknown> {
  const current = normalizeSectionDesign(blockType, propsJson);
  return patchPremiumDesign(propsJson, { ...current, ...patch } as Partial<NormalizedPremiumDesign>);
}

export const INSPECTOR_DESIGN_BLOCKS = new Set([
  'hero_campaign',
  'hero_form_campaign',
  'promo_autohall',
  'vehicle_offer',
