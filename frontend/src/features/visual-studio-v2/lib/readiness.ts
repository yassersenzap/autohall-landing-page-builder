...
import type { StudioV2RootProps } from '../design-tokens/types';
...
export function validateStudioV2Readiness(document: Data): ReadinessIssue[] {
  const issues: ReadinessIssue[] = [];
  const content = (document.content ?? []) as PuckNode[];
  const rootProps = document.root?.props as StudioV2RootProps | undefined;
