...
export function sanitizeCollectionItem(
  ...
  for (const field of schema.itemFields) {
    if (field.type === 'asset') {
      ...
      if (field.altKey && typeof raw[field.altKey] === 'string') {
        out[field.altKey] = trimString(String(raw[field.altKey]).trim());
      }
      continue;
    }
  ...
  for (const key of Object.keys(raw)) {
    ...
    if (schema.itemFields.some((field) => field.key === key)) continue;
  }

  const hasContent = schema.itemFields.some((field) => {
    const val = out[field.key];
    if (field.type === 'boolean') return true;
    if (field.type === 'string-list') return Array.isArray(val) && val.length > 0;
