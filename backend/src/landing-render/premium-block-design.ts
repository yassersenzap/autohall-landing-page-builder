/** Controlled design variants — hero_form_campaign & vehicle_offer (preview + export). */
...
export function normalizePremiumDesign(
  ...
  return {
    ...
    imageShape: pick(
      raw.imageShape,
      SHAPES,
      defaults.imageShape ?? 'rounded-card',
    ),
...
export function resolveHeroFormLayoutVariant(
  design: NormalizedPremiumDesign,
): string {
  if (design.variant === 'media-focus') {
    return design.mediaPosition === 'left'
      ? 'image_left_form_right'
      : 'text_left_form_right';
  }
  if (design.variant === 'compact') {
    return 'text_left_form_right';
  }
  return design.mediaPosition === 'left'
...
export function buildPremiumCtaClass(
  ...
  if (design.ctaStyle === 'outline') {
    return `${base} lp-btn--secondary`;
  }
  if (design.ctaStyle === 'white') {
    return `${base} lp-btn--white`;
  }
