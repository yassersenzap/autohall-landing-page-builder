export function typographyClassName(typography?: Record<string, unknown>): string {
  if (!typography || typeof typography !== 'object') return '';
  const parts: string[] = [];
  if (typeof typography.titleSize === 'string') parts.push(`vs2-typo-title-${typography.titleSize}`);
  ...
  if (typeof typography.textAlign === 'string') parts.push(`vs2-typo-align-${typography.textAlign}`);
  if (typography.textTransform === 'uppercase') parts.push('vs2-typo-uppercase');
  return parts.join(' ');
}

export function imageStyleClassName(imageStyle?: Record<string, unknown>): string {
  if (!imageStyle || typeof imageStyle !== 'object') return '';
  const parts: string[] = [];
  if (typeof imageStyle.imageFit === 'string') parts.push(`vs2-img-fit-${imageStyle.imageFit}`);
