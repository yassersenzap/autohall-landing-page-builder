...
function textClasses(props: Record<string, unknown>, base: string): string {
  ...
  const weight =
    typeof props.fontWeight === 'string' ? props.fontWeight : 'normal';
  const align = alignClass(props.alignment);
  const color =
    typeof props.colorPreset === 'string' ? props.colorPreset : 'default';
...
export function renderHeadingBlock(props: Record<string, unknown>): string {
  ...
  const level =
    props.level === 'h1' || props.level === 'h3' ? props.level : 'h2';
...
export function renderButtonBlock(props: Record<string, unknown>): string {
  ...
  const variant =
    typeof props.buttonStyle === 'string' ? props.buttonStyle : 'primary';
...
export function renderTextImageBlock(
  ...
  const layout =
    typeof props.layout === 'string' ? props.layout : 'image_right';
  const title =
...
export function renderCardBlock(props: Record<string, unknown>): string {
  const radius =
    typeof props.cardRadius === 'string' ? props.cardRadius : 'soft';
  const shadow =
    typeof props.cardShadow === 'string' ? props.cardShadow : 'soft';
  const align = alignClass(props.alignment);
  const title =
    typeof props.title === 'string' && props.title.trim()
...
export function renderQuoteBlock(props: Record<string, unknown>): string {
  ...
  const author =
    typeof props.author === 'string' ? escapeHtml(props.author) : '';
  const role =
    typeof props.role === 'string' && props.role.trim()
      ? escapeHtml(props.role.trim())
      : '';
  const align = alignClass(props.alignment);
  const footer = author
...
export function renderStatsBlock(props: Record<string, unknown>): string {
  ...
  const cards = items
    ...
    .map((item) => {
      const value =
        typeof item.value === 'string' ? escapeHtml(item.value) : '';
      const label =
        typeof item.label === 'string' ? escapeHtml(item.label) : '';
...
export function renderTestimonialsBlock(
  props: Record<string, unknown>,
): string {
  const title =
    typeof props.title === 'string' && props.title.trim()
  ...
  const cards = items
    ...
    .map((item) => {
      const quote =
        typeof item.quote === 'string' ? escapeHtml(item.quote) : '';
      const author =
        typeof item.author === 'string' ? escapeHtml(item.author) : '';
...
export function renderEventScheduleBlock(
  props: Record<string, unknown>,
): string {
  const title =
    typeof props.title === 'string' && props.title.trim()
...
export function renderFinancingHighlightBlock(
  props: Record<string, unknown>,
): string {
  const title =
    typeof props.title === 'string' && props.title.trim()
  ...
  const list = conditions
    ...
    .map(
      (c) => `<li>${typeof c.text === 'string' ? escapeHtml(c.text) : ''}</li>`,
    )
    .join('');
  const cta =
