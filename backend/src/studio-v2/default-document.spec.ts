...
import { buildDefaultStudioV2Document, STUDIO_V2_ENGINE } from './default-document';
...
describe('buildDefaultStudioV2Document', () => {
  ...
  it('builds nested section > container > columns structure with puck ids', () => {
    ...
    const section = (doc.content as Array<{ type: string; props: { id?: string } }>)[0];
  ...
  it('includes hero and lead form in default columns', () => {
    const doc = buildDefaultStudioV2Document();
    const sectionProps = (doc.content as Array<{ props: Record<string, unknown> }>)[0].props;
